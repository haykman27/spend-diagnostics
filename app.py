...
st.caption(
    ...
    "converts to EUR (latest ECB FX), and shows diagnostics in € k. If a key column isn’t recognized, "
...
def parse_price_to_float(x):
    ...
    s=re.sub(r"[^\d,\.\-]","",str(x))
    if "," in s and "." in s:
        s=s.replace(",","")               # comma as thousands sep
    elif "," in s and s.count(",")==1 and len(s.split(",")[-1]) in (2,3):
        s=s.replace(",",".")              # decimal comma
    else:
        s=s.replace(",","")               # plain thousands commas
    try: return float(s)
...
TARGETS={
 "category":[
     "category","category name","category desc","category description",
     "commodity","spend category","material group","material group desc",
     "gl category","family","family group","group","group desc","group name",
     "sub category","sub category desc","item family","item family group","item group",
...
if uploaded:
    ...
    # === Spend computation (prevent double FX) ===
    ...
    if spend_col:
        ...
        if _is_global_header(spend_col):
            ...
            if base_col:
                ...
                df["_spend_eur"]=pd.to_numeric(spend_vals,errors="coerce").fillna(0.0)  # already EUR
        else:
    ...
    st.dataframe(
        dbg[["supplier","category","Spend (€ k)","Calc (€ k)","consistency_ratio"]],
        use_container_width=True,
        column_config={
            "Spend (€ k)": st.column_config.NumberColumn(format="€ %d k"),
